...
\title{\vspace{-13mm}\includegraphics[width=15mm,scale=3]{../images/IST_Logo.png}\\ \vspace{0mm}
LFEUI - Investigation Proposal \\ {\fontsize{24}{16}Detection of Li in Technological Materials through Nuclear Reactions} \vspace{-5mm}}
\date{November 2023}
...
Before the emission of the reaction products, the reaction leads to the formation of a $\ce{^8Be}$ compound nucleus (intermediate state), which then rapidly decays into two alpha particles. These are the particles we detect.

According to the incident proton's energy ($E_p = 2.7 $ MeV) and the scattering angle of the detectors ($\theta = 165 $ degrees) we were able to use the NRA Calculator \cite{NRAEnergyCalc} to calculate the energy of the alpha particles resulting from the reaction as well as it's $Q$ value:
...
The detectors used are silicon based detectors - these detect the energy of incident charged particles through the ionisation they produce in the silicon.

In addition, they are connected to a electronic system that amplifies and formats the electric signal arising from individual interaction events and sends it to a multichannel analyser where we can see the collected energy spectra.
...
We are going to work with two different Lithium-7 samples: one of them is a glass sample composed of silicon, boron and lithium and the other ones will be silicon substrates implanted with $\ce{^7Li}$.
...
\begin{enumerate}
  ...
  \item Channel-energy calibration of the detection system with an alpha source of known composition;
...
particles. By obtaining the expected energy spectra we can infer the presence of Li in the samples. If possible we will also try
to quantify the amount of Li present by analysing the yield of the $\ce{^7Li}$(p,$\ce{^4He})\ce{^4He}$ nuclear reaction.
...
From the glass sample we expect to see a continuous nuclear reaction yield and from the implanted samples we only expect to see well defined nuclear reaction yields

\paragraph{}

Other interaction phenomena such as Rutherford Backscattering and Elastic Scattering will also be visible in the spectra. These are responsible for energy peaks in both sample's spectra at lower than the expected energy values.
...
Until now, the only way to do this was through atomic spectroscopy techniques, which, besides not being able to detect the presence of lighter elements, they also present lower sensitivities for elemental quantification.
