...
We decided to do the analysis only with "detector 0". This analysis could be further corroborated using the "detector 1" data.
Using the triple gaussian, we obtained the following fit for "detector 0":
...
Where $N\tau_i$ are $N\Delta x$ different values of the superfitial concentration of the element. For comparison, we can approximate $N\Delta x \approx N\tau_i \times$\#channels.

$H_i$ is the hight, in number of counts, in a spectrum with accumulation (used in the $\ce{LiAlO2}$ and $\ce{LiF}$ samples), $A$ is the area of a singular peak (used in the implanted sample), $\sigma (E_i) $ is the cross section of the reaction, $\Omega$ is the solid angle of the detector, $Q$ is the max charge acquisition divided by $e$.
...
analysed samples trough nuclear reactions. Furthermore, we managed to go beyond our proposal and extimate the amount of 
