...
\begin{abstract}
  ...
  The research was conducted in three different samples: lithium fluoride ($\ce{LiF}$), lithium aluminate ($\ce{LiAlO2}$) and an implanted sample. 
  ...
  Also, some other phenomena were observed, such as back-scattering, pile-up and energy profiling.
  Finally, using the information on each spectrum, we computed the quantity of lithium in each sample.
\end{abstract}
\vspace{-0.1cm}
...
We started by displaying the samples in a vertical sample holder with the given order: in the lowest position the implanted sample, followed by lithium aluminate ($\ce{LiAlO2}$) and finally lithium fluoride ($\ce{LiF}$). In order to do a calibration from the number of counts per channel to energy, we added an additional source on the holder just above the lithium fluoride one. Using a pachymeter, we measured the distance between the samples so that afterwards we could control the hight in order to change the sample on the beam's target.
...
Finally, we used a computer program to obtain the spectra of the different sources. The multichannel analyzer (MCA) used to acquire data was gradually set to 70 V and the angle between the top two detectors was 165º. 
...
We separated the spectra of channel/detectors 0, 1 and 2 (2 corresponding to the bottom one) and used a triple gaussian sum function to fit on the three energy peaks. According to theory, the first peak corresponds to the decay of plutonium into uranium, the second of americium into neptunium and the third of curium into plutonium. The decay equations are:
...
We decided to do the analysis only with detector/channel 0. This analysis could be further corroborated using the "detector 1" data.
Using the triple gaussian, we obtained the following fit for channel 0:
...
Using the values in the table we now have to compute a calibration equation for channel/detector 0.
We plotted the theoretical values of the energy of the three peaks as a function of fit parameters:\vspace{-2mm}

\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/Chn0_calib_with_Backscattering.png}
...
DE ONDE VEM ISTO CARAGO? TEMOS DE TER REFERÊNCIAS

\begin{center}
    \label{alfa_peaks}
    \centering
    \includegraphics[scale = 0.3]{../../images/AlphaPeaks.png}
...
Since we know the emited alpha in our samples is more energetic than the theoretical values, it is expected that the experimental peaks are slightly higher than the theoretical ones.
If the samples contains a peak in the proximity of the alpha peaks it shows that the sample contains lithium.
...
\begin{center}
    \centering
    \includegraphics[scale = 0.3]{../../images/OverlapLiF_NC.png}
...
\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/FullAnalysisLiF.png}
...
\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/OverlapLiAlO2.png}
...
\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/FullAnalysisLiAlO2.png}
...
\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/OverlapImplanted.png}
...
\begin{center}
    \label{TT_21}
    \centering
    \includegraphics[scale = 0.3]{../../images/FullAnalysisImplanted.png}
...
To conclude, we ended up with a successful experiment. We were able to detect the presence of lithium in all the 
analysed samples trough nuclear reactions. Furthermore, we managed to go beyond our proposal and estimate the amount of 
